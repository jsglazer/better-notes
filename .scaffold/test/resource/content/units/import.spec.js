...
`;
  ...
  // test/utils/wait.ts
  async function waitNoMoreThan(promise, timeout = 3e3, message = "Timeout") {
    let resolved = false;
    return Promise.any([
      promise.then((result) => {
        resolved = true;
  ...
  function waitForItemModifyEvent(itemID, timeout = 3e3) {
  ...
  describe("Import", function() {
    ...
    it("api.$import.fromMD", async function() {
      ...
      await wait_exports.waitUtilAsync(() => {
      ...
      editor?.saveSync();
      await Promise.race([itemModifyPromise, Zotero.Promise.delay(1e3)]);
      const content = note.getNote();
      const expected = getNoteContent().replace(
        `<p>Item 2</p>
<p>Continuation of item 2 without proper indentation.</p>`,
        "Item 2Continuation of item 2 without proper indentation."
      );
