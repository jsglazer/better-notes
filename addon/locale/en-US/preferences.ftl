...
editor-readableWidth =
    .label = Limit text to a comfortable reading width
editor-headingCollapse =
    .label = Collapse sections by clicking a heading
editor-codeHighlight =
    .label = Syntax highlighting in code blocks
editor-callouts =
    .label = Render Obsidian-style callouts ([!note], [!warning], …)
editor-mathPreview =
    .label = Render formulas with KaTeX in the editor
editor-markdownInputRules =
    .label = Markdown shortcuts while typing (## for a heading, **bold**)
editor-featureToggles-description = Reading width applies immediately; the others take effect in notes you open next. If the editor ever misbehaves, switch these off to rule them out.
...
sync-externalEditQuiet-label = Leave a file alone for (seconds)
sync-externalEditQuiet-description = How long a Markdown file must sit unchanged before syncing will touch it. Editors such as Obsidian save every couple of seconds while you type; syncing into that window makes them reload the note and can misplace the keystrokes in flight. 0 turns the protection off.
sync-manager =
